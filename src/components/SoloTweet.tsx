...
export function SoloTweet({ tweet, userOn }: any) {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5000/comments/${tweet.id}`)
      .then((resp) => resp.json())
      .then((data) => {
        setComment(data);
      });
  ...
  return (
    <div className="tweet">
      ...
      <div className="tweet-actions">
        ...
        {retweeted ? (
          ...
          <div
            ...
            onClick={(e) => {
              fetch(`http://localhost:5000/retweet/${userOn.id}/${tweet.id}`, {
                method: "POST",
      ...
      <div className="tweet-your-reply">
        ...
        <form
          onSubmit={(e) => {
            ...
            if (e.target.comment.value.length >= 5) {
              fetch(`http://localhost:5000/comment/${userOn.id}/${tweet.id}`, {
                method: "POST",
                headers: {
        ...
        >
          <input type="text" placeholder="Tweet your reply" name="comment" />
        </form>
      </div>
